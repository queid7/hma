...
class OdeModelRenderer(Renderer):
    ...
    def render(self):
        glColor3ubv(self.totalColor)
        for node in list(self.model.nodes.values()):
            geom = node.geom
...
#        glMultMatrixf(mm.r_to_t(posture.localRMap[joint.name]).transpose())
        glMultMatrixf(mm.r_to_t(posture.localRs[posture.skeleton.getElementIndex(joint.name)]).transpose())
...
#            self.rc.drawPoint((0,0,0))
            ...
            if len(joint.children) > 0:
                ...
                boneT = mm.r_to_t(mm.getSO3FromVectors(defaultBoneV, offset))
...
class MMMotionRenderer(Renderer):
    ...
    def render(self):
        ...
        self.renderPointPosture(posture)
    def renderPointPosture(self, posture):
        for name, point in list(posture.pointMap.items()):
#            if name in self.partColors:
#                glColor3ubv(self.partColors[name])
#            else:
#                glColor3ubv(self.totalColor)
...
#                randomSize = 100
#                variation = mm.vec3(randomSize*random.random(), randomSize*random.random(), randomSize*random.random())
...
#        self.colors = [self.totalColor]*len(self.bodies)
        self.colors = [mm.seq_to_vec3(self.totalColor)*(1-(float(i)/len(self.bodies) * .5)) for i in range(len(self.bodies))]
...
#        glPolygonMode(GL_FRONT_AND_BACK, GL_FILL)
        ...
        arrowT = mm.r_p_to_t(mm.getSO3FromVectors((length,0,0), vector), startPos)
    ...
    def drawCircularArrow(self, startPos, endPos, rotVec=None, lineWidth=.02, radius=.1):
        ...
        axisT = mm.r_p_to_t(mm.getSO3FromVectors((0,0,length), rotVec), startPos)
...
if __name__=='__main__':
    ...
    def test_FramesRenderer_OrientationsRenderer():
        ...
        viewer.doc.addRenderer('orientation0', OrientationsRenderer([mm.t_to_r(frame0)], [mm.t_to_p(frame0)], (0,255,0)))
        viewer.doc.addRenderer('orientation1', OrientationsRenderer([mm.t_to_r(frame1)], [mm.t_to_p(frame1)], (0,255,0)))

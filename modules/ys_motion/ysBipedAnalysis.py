...
#        steps = getWalkingSteps(lc, rc, False)
        ...
        for i in range(len(steps)):
            print('stepMotions[%d]: ys_motion[%d]~ys_motion[%d], len %d'%(i, steps[i][0], steps[i][1], len(stepMotions[i])))
        
        viewer = ysv.SimpleViewer()
        viewer.record(False)
        viewer.doc.addRenderer('ys_motion', yr.JointMotionRenderer(motion, (0,100,255), yr.LINK_LINE, 3.))

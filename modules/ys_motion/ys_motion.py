...
class JointMotion(Motion):
    ...
    # p: vec3(position), R : seq_to_so3(orientation)
...
#        R, p = mm.t_to_r_p(DOFPositions[0])
...
#        return map(lambda R0,R1: (self.fps/(frame1-frame0)) * mm.log_so3(np.dot(R0.transpose(), R1)), orientations0, orientations1)
    ...
    def rotateTrajectory(self, R_offset, fixedPos=None, update=True):
        if fixedPos is None: fixedPos = self[0].rootPos
        for p in self:
...
#            self.globalTs[i] = mm.i_se3()
    ...
    def getGlobalR(self, index):
        return mm.t_to_r(self.globalTs[index])
...
#        return mm.t_to_p(self._calcGlobalT(self.skeleton.getElement(index)))
...
#        else:
#            T = mm.p_to_t(self.rootPos)
#        T = np.dot(T, mm.p_to_t(joint.offset))
#
#        index = self.skeleton.getElementIndex(joint.name)
#        T = np.dot(T, mm.r_to_t(self.localRs[index]))
#        return T
...
#                R, p = mm.t_to_r_p(self.globalTs[parent])
...
#        self.globalTs[index] = mm.r_p_to_t(R, p)
#        for childJoint in joint.children:
#            self._updateGlobalT(childJoint, p, R)
            
    def updateGlobalT(self, fromIndex=None):
        if fromIndex is None:
            self._updateGlobalT(self.skeleton.root, mm.p_to_t(self.rootPos))
        else:
            parent = self.skeleton.getParentIndex(fromIndex)
            if parent is None:
                self._updateGlobalT(self.skeleton.root, mm.p_to_t(self.rootPos))
    ...
    def _updateGlobalT(self, joint, parentT):
        index = self.skeleton.getElementIndex(joint.name)
        T = np.dot(parentT, mm.p_to_t(joint.offset))
        T = np.dot(T, mm.r_to_t(self.localRs[index]))
...
#                self._updateGlobalT(childJoint, T)
    ...
    def getPosition(self, index):
        return mm.t_to_p(self.globalTs[index])
    def getPositions(self):
...
class JointDisplacement(JointPosture):
    ...
    def __mul__(self, constant):
        ...
        for i in range(len(self.localRs)):
            R = self.localRs[i]
#            R_d = mm.exp(constant * mm.log_so3(R))
...
if __name__ == '__main__':
    ...
    def test_getGlobalT():    
        ...
        def preFrameCallback(frame):
            jointMotion[frame].mulLocalR(lul, mm.exp(mm.vec3(0,0,1), math.pi/4.))
...
#        motion_setslice[0:0] = ys_motion
        
        viewer = ysv.SimpleViewer()
        viewer.record(False)
        viewer.doc.addRenderer('ys_motion', yr.JointMotionRenderer(motion, (0,0,255)))
...
#        motions = [pointMotion, jointMotion]
    ...
    def test_blendPosture():
        ...
        viewer = ysv.SimpleViewer()
        viewer.record(False)
        viewer.doc.addRenderer('ys_motion', yr.JointMotionRenderer(jointMotion, (0,255,0), yr.LINK_LINE))
        viewer.doc.addObject('ys_motion', jointMotion)
...
#        ys_motion = yf.readBvhFile('../samples/wd2_WalkSameSame00.bvh', .01)
#        ys_motion = yf.readBvhFile('../samples/block_3_rotate.bvh', 1)
        ...
        viewer = ysv.SimpleViewer()
        viewer.record(False)
        viewer.doc.addRenderer('ys_motion', yr.JointMotionRenderer(motion, (0,0,255), yr.LINK_WIREBOX))
        viewer.doc.addObject('ys_motion', motion)
...
#        yme.rotateJointLocal(ys_motion, 0, mm.exp(mm.vec3(0,1,0), math.pi/2))
...
#            ap[:] = map(mm.log_so3, ys_motion.getJointOrientationsGlobal(frame))
            av[:] = motion.getJointAngVelocitiesGlobal(frame)
#            av[:] = ys_motion.getJointAngVelocitiesLocal(frame)

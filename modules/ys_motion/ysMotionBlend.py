...
#        R_offset = d.getJointOrientationLocal(0)
        ...
        if ori_xz_preserve:
#            R_offset_y, R_offset_xz = mm.project_rotation((0,1,0), R_offset_global)
            R_offset_y, R_offset_xz = mm.projectRotation2((0,1,0), R_offset_global)
            R_offset_global = R_offset_y
#            R_offset_y, R_offset_xz = mm.project_rotation((0,1,0), R_offset)
#            R_offset_y, R_offset_xz = mm.project_rotation(np.dot(alignRefPosture_or_d.localRs[0], (0,1,0)), R_offset)
...
#        d.disableTranslation()
    ...
    if attachOrientation:
        ...
        R_offset_A = mm.exp(mm.projectionOnVector(mm.logSO3(R_offset_A), mm.vec3(0,1,0))) # # project on y axis
        R_offset_B = dA.getJointOrientationLocal(0)
        R_offset_B = mm.exp(mm.projectionOnVector(mm.logSO3(R_offset_B), mm.vec3(0,1,0))) # # project on y axis
#        d.disableRotations([0])
...
def blendSegmentSmooth(motionSegment0, motionSegment1, attachPosition=True, attachOrientation=True):
    ...
    if attachOrientation:
        ...
        R_offset = mm.exp(mm.projectionOnVector(mm.logSO3(R_offset), mm.vec3(0,1,0))) # # project on y axis
...
#        bvhFilePath = '../samples/wd2_WalkSukiko00.bvh'
    ...
    def test_early_foottouch():
        ...
        print('ys_motion[%d]~ys_motion[%d] => '%(steps[earlyCycle][0], steps[earlyCycle][1]), end=' ')
...
#            stitched = getStitchedNextMotion(stepMotions[i], motion_earlytouch[-1], len(stepMotions[i])-1, yfg.identity)
        ...
        viewer = ysv.SimpleViewer()
        viewer.record(False)
        viewer.doc.addRenderer('ys_motion', yr.JointMotionRenderer(motion, (100,100,255), yr.LINK_LINE))
        viewer.doc.addObject('ys_motion', motion)
    ...
    def test_late_foottouch():
        ...
        print('ys_motion[%d]~ys_motion[%d] => '%(steps[lateCycle][0], steps[lateCycle][1]), end=' ')
